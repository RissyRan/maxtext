...
def decode_loop(config, state=None):
  ...
  kv_cache_annotations = max_utils.get_kv_cache_annotations(
      model, config, rng, mesh)

  assert state.opt_state == {}, "non null opt_state in checkpoint"
  num_params, bytes_params, bytes_per_param = max_utils.summarize_size_from_pytree(
      state.params)
  ...
  with jax.spmd_mode('allow_all'):
    ...
    num_cache, bytes_cache, bytes_per_cache = max_utils.summarize_size_from_pytree(
        prefill_cache)
    ...
    max_logging.log(
        f"Total memory (for cache and params) {total_memory_GB:.3f} GB")

    partial_ar_predict_step = functools.partial(
        ar_predict_single_token, model=model)
    ...
    p_ar_predict_step = jax.jit(
        ...
        in_shardings=(replicated_sharding, replicated_sharding,
                      kv_cache_mesh_shardings, state_mesh_shardings, None),
    ...
    outputs = []
    first_profiling_step = config.max_prefill_predict_length + \
        config.skip_first_n_steps_for_profiler
    last_profiling_step = np.clip(first_profiling_step + config.profiler_steps - 1,
                                  first_profiling_step, config.max_target_length - 1)

    new_position, new_cache, new_id = p_ar_predict_step(
        new_id, new_position, new_cache, state, rng)
    ...
    memory_bandwidth_per_device_GB_per_sec = total_memory_GB / \
        (elapsed_time/steps)/jax.device_count()
